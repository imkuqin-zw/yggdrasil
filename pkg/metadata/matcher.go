...
// mdValuesFromOutCtx retrieves metadata from context. If there are
...
func mdValuesFromOutCtx(md MD, key string) (string, bool) {
	vs, ok := md[key]
	if !ok {
		return "", false
	}
...
// HeaderExactMatcher.
func (hem *HeaderExactMatcher) Match(md MD) bool {
	v, ok := mdValuesFromOutCtx(md, hem.key)
	if !ok {
		return false
	}
...
// Match returns whether the passed in HTTP Headers match according to the
// HeaderRegexMatcher.
func (hrm *HeaderRegexMatcher) Match(md MD) bool {
	v, ok := mdValuesFromOutCtx(md, hrm.key)
	if !ok {
		return false
	}
	return xstrings.FullMatchWithRegex(hrm.re, v) != hrm.invert
}
...
// Match returns whether the passed in HTTP Headers match according to the
// HeaderRangeMatcher.
func (hrm *HeaderRangeMatcher) Match(md MD) bool {
	v, ok := mdValuesFromOutCtx(md, hrm.key)
	if !ok {
		return false
	}
...
func (hpm *HeaderPresentMatcher) Match(md MD) bool {
	vs, ok := mdValuesFromOutCtx(md, hpm.key)
...
// Match returns whether the passed in HTTP Headers match according to the
// HeaderPrefixMatcher.
func (hpm *HeaderPrefixMatcher) Match(md MD) bool {
	v, ok := mdValuesFromOutCtx(md, hpm.key)
	if !ok {
		return false
	}
...
// Match returns whether the passed in HTTP Headers match according to the
// HeaderSuffixMatcher.
func (hsm *HeaderSuffixMatcher) Match(md MD) bool {
	v, ok := mdValuesFromOutCtx(md, hsm.key)
	if !ok {
		return false
	}
...
func (hcm *HeaderContainsMatcher) Match(md MD) bool {
	v, ok := mdValuesFromOutCtx(md, hcm.key)
	if !ok {
		return false
	}
...
type HeaderStringMatcher struct {
	key           string
	stringMatcher xstrings.StringMatcher
	invert        bool
}

// NewHeaderStringMatcher returns a new HeaderStringMatcher.
func NewHeaderStringMatcher(key string, sm xstrings.StringMatcher, invert bool) *HeaderStringMatcher {
...
func (hsm *HeaderStringMatcher) Match(md MD) bool {
	v, ok := mdValuesFromOutCtx(md, hsm.key)
	if !ok {
		return false
	}
