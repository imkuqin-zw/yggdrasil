...
func (c *polaris) Read() (types.ConfigSourceData, error) {
	var err error
	...
	return source.NewBytesSourceData(types.ConfigPriorityRemote, content, yaml.Unmarshal), nil
}
...
func (c *polaris) Watch() (<-chan types.ConfigSourceData, error) {
	csdCh := make(chan types.ConfigSourceData, 1)
		...
		for {
			...
			case event := <-changeEventChan:
				...
				if err := yaml.Unmarshal(content, &kv); err != nil {
					log.Warnf("fault to unmarshal config, err: %s", err.Error())
					continue
				}
				...
				csdCh <- source.NewBytesSourceData(types.ConfigPriorityRemote, content, yaml.Unmarshal)
			case _, _ = <-c.closeCh:
				return
			}
		}
...
func NewConfig(namespace, group, filename string) (types.ConfigSource, error) {
	c, err := Context()
	if err != nil {
		return nil, err
	}
...
func LoadConfig(appName string) error {
	...
	namespace := config2.Get("yggdrasil.application.namespace").String("default")
	var sources []types.ConfigSource
