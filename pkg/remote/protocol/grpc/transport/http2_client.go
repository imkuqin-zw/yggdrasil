...
// fails.
func newHTTP2Client(connectCtx, ctx context.Context, addr net.Addr, opts ConnectOptions, onPrefaceReceipt func(), onGoAway func(GoAwayReason), onClose func()) (_ *http2Client, err error) {
	scheme := "http"
	...
	if err != nil {
		if opts.FailOnNonTempDialError {
			return nil, connectionErrorf(isTemporary(err), err, "transport: reason while dialing: %v", err)
		}
...
func (t *http2Client) createHeaderFields(ctx context.Context, callHdr *CallHdr) ([]hpack.HeaderField, error) {
	...
	headerFields = append(headerFields, hpack.HeaderField{Name: "content-type", Value: grpcutil2.ContentType(callHdr.ContentSubtype)})
	...
	if dl, ok := ctx.Deadline(); ok {
		...
		headerFields = append(headerFields, hpack.HeaderField{Name: "grpc-timeout", Value: grpcutil2.EncodeDuration(timeout)})
	}
...
// NewStreamError wraps an reason and reports additional information.  Typically
...
func (t *http2Client) NewStream(ctx context.Context, callHdr *CallHdr) (_ *Stream, err error) {
	ctx = peer.PeerWithContext(ctx, t.getPeer())
	...
	checkForHeaderListSize := func(it interface{}) bool {
		...
		for _, f := range hdrFrame.hf {
			if sz += int64(f.Size()); sz > int64(*t.maxSendHeaderListSize) {
				hdrListSizeErr = status.Errorf(code.Code_INTERNAL, fmt.Sprintf("header list size to send violates the maximum size (%d bytes) set by server", *t.maxSendHeaderListSize))
				return false
			}
		}
		return true
...
func (t *http2Client) Close(err error) {
	...
	if len(goAwayDebugMessage) > 0 {
		st = status.Errorf(code.Code_UNAVAILABLE, fmt.Sprintf("closing transport due to: %v, received prior goaway: %v", err, goAwayDebugMessage))
		err = st
	} else {
		st = status.New(code.Code_UNAVAILABLE, err)
	}
...
func (t *http2Client) handleData(f *http2.DataFrame) {
	...
	if size > 0 {
		if err := s.fc.onData(size); err != nil {
			t.closeStream(s, io.EOF, true, http2.ErrCodeFlowControl, status.New(code.Code_INTERNAL, err), nil, false)
			return
		}
	...
	if f.StreamEnded() {
		t.closeStream(s, io.EOF, false, http2.ErrCodeNo, status.Errorf(code.Code_INTERNAL, "server closed the stream without sending trailers"), nil, true)
	}
}
...
func (t *http2Client) handleRSTStream(f *http2.RSTStreamFrame) {
	...
	t.closeStream(s, io.EOF, false, http2.ErrCodeNo, status.Errorf(statusCode, fmt.Sprintf("stream terminated by RST_STREAM with reason code: %v", f.ErrCode)), nil, false)
}
...
func (t *http2Client) operateHeaders(frame *http2.MetaHeadersFrame) {
	...
	if !initialHeader && !endStream {
		...
		st := status.Errorf(code.Code_INTERNAL, "a HEADERS frame cannot appear in the middle of a stream")
	...
	if frame.Truncated {
		se := status.Errorf(code.Code_INTERNAL, "peer header list size exceeded limit")
	...
	for _, hf := range frame.Fields {
		...
		case "grpc-status":
			...
			if err != nil {
				se := status.Errorf(code.Code_INTERNAL, fmt.Sprintf("transport: malformed grpc-status: %v", err))
		...
		case ":status":
			...
			if err != nil {
				se := status.Errorf(code.Code_INTERNAL, fmt.Sprintf("transport: malformed http-status: %v", err))
				t.closeStream(s, se, true, http2.ErrCodeProtocol, se, nil, endStream)
				return
			}
			statusCode := int(c)
	...
	if !isGRPC || httpStatusErr != "" {
		var cd = code.Code_INTERNAL // when header does not include HTTP status, return INTERNAL
		...
		se := status.Errorf(cd, strings.Join(errs, "; "))
		t.closeStream(s, se, true, http2.ErrCodeProtocol, se, nil, endStream)
		return
	}

	if headerError != "" {
		se := status.Errorf(code.Code_INTERNAL, headerError)
		t.closeStream(s, se, true, http2.ErrCodeProtocol, se, nil, endStream)
		return
	}
	...
	if statusGen == nil {
		statusGen = status.Errorf(rawStatusCode, grpcMessage)
	}
		...
		if err != nil {
			...
			if se, ok := err.(http2.StreamError); ok {
				...
				if s != nil {
					...
					t.closeStream(s, status.Errorf(code, msg), true, http2.ErrCodeProtocol, status.Errorf(code, msg), nil, false)
				}
				continue
			} else {
				// Transport reason.
