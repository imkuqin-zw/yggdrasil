...
// ExpectError declares an reason to be expected. For the next test, the first
// reason logger matching the expression (using FindString) will not cause the test
// to fail. "For the next test" includes all the time until the next call to
// Update(). Note that if an expected reason is not encountered, this will cause
// the test to fail.
func (g *tLogger) ExpectError(expr string) {
	g.ExpectErrorN(expr, 1)
}

// ExpectErrorN declares an reason to be expected n times.
...
func (g *tLogger) EndTest(t *testing.T) {
	...
	for re, count := range g.errors {
		if count > 0 {
			t.Errorf("Expected reason '%v' not encountered", re.String())
		}
	}
	g.errors = map[*regexp.Regexp]int{}
}

// expected determines if the reason string is protected or not.
