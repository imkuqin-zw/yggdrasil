...
func (h *testStreamHandler) handleStream(t *testing.T, s *Stream) {
	...
	if !bytes.Equal(p, req) {
		...
		h.t.WriteStatus(s, status.Errorf(code.Code_INTERNAL, "panic"))
		return
	}
	// send a response back to the client.
	h.t.Write(s, nil, resp, &Options{})
	// send the trailer to end the stream.
	h.t.WriteStatus(s, status.Errorf(code.Code_OK, ""))
}
...
func (h *testStreamHandler) handleStreamDelayRead(t *testing.T, s *Stream) {
	...
	// send the trailer to end the stream.
	if err := h.t.WriteStatus(s, status.Errorf(code.Code_OK, "")); err != nil {
...
func (s *server) start(t *testing.T, port int, serverConfig *ServerConfig, ht hType) {
	...
	for {
		...
		case suspended:
			go transport.HandleStreams(context.TODO(), func(*Stream) {}) // Do nothing to handle the stream.
		case misbehaved:
...
func setUpWithNoPingServer(t *testing.T, copts ConnectOptions, connCh chan net.Conn) (*http2Client, func()) {
	...
	connectCtx, cancel := context.WithDeadline(context.Background(), time.Now().Add(2*time.Second))
	tr, err := NewClientTransport(connectCtx, context.Background(), lis.Addr(), copts, func() {}, func(GoAwayReason) {}, func() {})
	if err != nil {
		cancel() // Do not cancel in success path.
		// Server clean-up.
		lis.Close()
...
func (s) TestInflightStreamClosing(t *testing.T) {
	...
	serr := status.Errorf(code.Code_INTERNAL, "client connection is closing")
	go func() {
		defer close(donec)
...
func (s) TestLargeMessageSuspension(t *testing.T) {
	...
	expectedErr := status.Errorf(code.Code_DEADLINE_EXCEEDED, context.DeadlineExceeded.Error())
...
func (s) TestClientWithMisbehavedServer(t *testing.T) {
	...
	connectCtx, cancel := context.WithDeadline(context.Background(), time.Now().Add(2*time.Second))
	defer cancel()
	ct, err := NewClientTransport(connectCtx, context.Background(), lis.Addr(), ConnectOptions{}, func() {}, func(GoAwayReason) {}, func() {})
	if err != nil {
...
var encodingTestStatus = status.Errorf(code.Code_INTERNAL, "\n")
...
func (s) TestInvalidHeaderField(t *testing.T) {
	...
	if se, ok := status.CoverError(err); !ok || se.Code() != int32(code.Code_INTERNAL) || !strings.Contains(err.Error(), expectedInvalidHeaderField) {
		t.Fatalf("Read got reason %v, want reason with code %s and contains %q", err, code.Code_INTERNAL, expectedInvalidHeaderField)
	}
	ct.Close(fmt.Errorf("closed manually by test"))
	server.stop()
}
...
func (s) TestContextErr(t *testing.T) {
	for _, test := range []struct {
		...
		{context.DeadlineExceeded, status.Errorf(code.Code_DEADLINE_EXCEEDED, context.DeadlineExceeded.Error())},
		{context.Canceled, status.Errorf(code.Code_CANCELLED, context.Canceled.Error())},
	} {
...
func (s) TestReadGivesSameErrorAfterAnyErrorOccurs(t *testing.T) {
	...
	if actualErr.Error() != testErr.Error() {
		t.Errorf("_ , actualErr := s.Read(_) differs; want actualErr.Status() to be %v; got %v", testErr.Error(), actualErr.Error())
	}
...
func (s) TestClientDecodeHeaderStatusErr(t *testing.T) {
	...
	for _, test := range []struct {
	...
	}{
		{
			...
			// no reason
			wantStatus: status.Errorf(code.Code_OK, ""),
		},
		{
